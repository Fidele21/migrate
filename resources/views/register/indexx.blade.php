...
<div class="page-head">
  <div>
    <div class="crumb">{{ $type['group'] }} &middot; Register</div>
    <h2>{{ $type['name'] }} &mdash; Inspection Register</h2>
  </div>
...
<form method="get" class="fbar" id="fpanel">
  <div class="fb-id">
    <span class="fb-crumb">{{ $type['group'] ?? 'Register' }}</span>
    <span class="fb-name">{{ $type['name'] }}</span>
    <span class="fb-scope">
      {{ $rows->count() }} {{ Str::plural('premises', $rows->count()) }} listed
    </span>
  </div>
  ...
  <div class="fb-fields">
    ...
    {{-- Deliberation --}}
    <div class="fb-group fb-wide">
      <label for="deliberation">Deliberation</label>
      <select name="band" id="deliberation">
        <option value="">All outcomes</option>
        @foreach(['permanent', 'temporary', 'improve', 'compliant'] as $k)
          <option value="{{ $k }}" @selected(($filters['band'] ?? '') === $k)>
            {{ \App\Support\ComplianceBand::BANDS[$k]['label'] }}
          </option>
        @endforeach
      </select>
    </div>
    ...
    {{-- Visits --}}
    <div class="fb-group">
      <label for="visits">Inspections</label>
      <select name="visits" id="visits">
        <option value="">Any number</option>
        <option value="once"   @selected(($filters['visits'] ?? '') === 'once')>Once only</option>
        <option value="repeat" @selected(($filters['visits'] ?? '') === 'repeat')>More than once</option>
        <option value="3"      @selected(($filters['visits'] ?? '') === '3')>Three or more</option>
      </select>
    </div>
...
{{-- Stats --}}
<div class="stats">
  <div class="stat"><b>{{ $summary['premises'] }}</b><span>Premises listed</span></div>
  <div class="stat t"><b>{{ $summary['inspections'] }}</b><span>Inspections</span></div>
  <div class="stat g"><b>{{ $summary['mean'] }}%</b><span>Mean compliance</span></div>
  <div class="stat y"><b>{{ $summary['repeat'] }}</b><span>Visited more than once</span></div>
  <div class="stat r"><b>{{ $summary['below70'] }}</b><span>Below 70%</span></div>
</div>

<section>
  <h3>Register</h3>
  <div class="desc">{{ $summary['premises'] }} {{ Str::plural('premises', $summary['premises']) }} matching the current filters</div>

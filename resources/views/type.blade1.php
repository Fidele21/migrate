  ...
  <section class="panel area-analysis">
      ...
      <div class="p-head compact">
        <div>
          ...
          <div class="p-desc">Stations sited where regulations do not permit one, whatever their score</div>
        </div>
        <div class="p-tools">
        ...
        <span class="pc-headline-label">{{ Str::plural('station', $permanentStations->count()) }} permanently closed</span>
        <span class="pc-headline-more">Why &rsaquo;</span>
      </button>
      ...
      <div class="pc-modal-overlay" id="pc-modal-overlay" style="display:none">
        <div class="pc-modal" role="dialog" aria-modal="true">
          <div class="pc-modal-head">
            <h3>Why stations are permanently closed</h3>
          ...
          <div class="pc-modal-body">
            <div class="pc-modal-total">
              ...
              <span>petrol {{ Str::plural('station', $permanentStations->count()) }} currently permanently closed</span>
            </div>
...
<style>
  ...
  .area-list{flex:1;overflow-y:auto;min-height:0}
  ...
  /* ---- Premises ranked, as columns ---- */
  .rank-scroll{flex:1;min-height:0;overflow-x:auto;overflow-y:hidden}
  .rank-scroll::-webkit-scrollbar{height:6px}
  .rank-scroll::-webkit-scrollbar-thumb{background:var(--border)}
  ...
  .pc-bar-row{display:flex;align-items:center;gap:10px;margin-bottom:10px}
  .pc-bar-label{width:150px;font-size:12px;color:var(--body-text);flex-shrink:0}
  ...
  .pc-bar-n{width:24px;text-align:right;font-family:var(--f-head);font-weight:700;font-size:13px;color:var(--ink)}
  .pc-bar-note{font-size:10.5px;color:var(--tertiary);font-style:italic;margin-top:6px}
...
(function () {
  ...
  /* ---- Districts, then sectors ----
     Bars for how many premises were inspected, a line for how well they
     scored. Sorted weakest first, so the areas needing attention lead.

     Two scales because the figures are not comparable: a count of 40 and
     a rate of 40% share a number and nothing else. */
  (function () {
  ...
  drawSections('bar');
  /* ---- Premises ranked ----
     One column per premises: what it achieved, and what it still owes.
     A percentage alone hides the size of the gap; stacking it against
     the shortfall does not. */
  (function () {
...
/* ═══════ Date range — its own closure ═══════
   Kept separate so a fault anywhere else on the page cannot stop it.
   This control failed silently once because a script above it threw. */
(function () {
